"""Smoke tests for the ported @oracle_link island registry (c64_re/islands.py)."""
...
from c64_re.islands import OracleLink, collect_islands, oracle_link, render_manifest  # noqa: E402

_BOUNDARY = re.compile(r"^\$[0-9A-Fa-f]{4}$")


def test_oracle_link_attaches_metadata_and_returns_function_unchanged():
    @oracle_link("$348D", "draws one tile row into the work screen",
...
def test_oracle_link_rejects_unknown_status():
    with pytest.raises(ValueError):
        oracle_link("$0001", "x", status="PROBABLY_FINE")
...
def _make_fake_adapter_package(tmp_path):
    ...
    (pkg / "recovered" / "rules.py").write_text(textwrap.dedent("""
        from c64_re.islands import oracle_link

        @oracle_link("$D007", "advances one game tick", status="ASM_MATCHED",
                     merge_target="game loop")
        def tick(state):
            return state

        @oracle_link("$26FA", "draws the object list", merge_target="renderer")
...
def test_collect_and_render_manifest_discovers_decorated_functions(tmp_path):
    ...
    try:
        ...
        assert names == [("draw_objects", "$26FA"), ("tick", "$D007")]

        manifest = render_manifest((f"{pkg_name}.recovered",))
        assert "| `$D007` | `recovered.rules.tick` | ASM_MATCHED | game loop |" in manifest
