"""Tests for c64_re.coverage — the measured native-% collector.

Game-free: a toy classifier over synthetic addresses; events fed directly (the CPU/verifier wiring
only calls these four methods — verified calls carry ``VerifyOutcome.oracle_instructions``)."""
...
from c64_re.coverage import CoverageCollector, fmt_addr  # noqa: E402


def classify(addr, name=""):
    if "decode" in name:
        return "codecs"
    return "gameplay" if addr < 0x8000 else "render"
