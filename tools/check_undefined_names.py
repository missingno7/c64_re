...
Usage:
    python tools/check_undefined_names.py [package_dir]     # default: c64_re
...
DOS port (where this tool originates) two such latent ``NameError`` landmines shipped
this way, each hidden in a recovered-code path with no oracle coverage.
...
Origin: ported as-is from dos_re's ``tools/check_undefined_names.py``.
"""
...
PKG = ROOT / (sys.argv[1] if len(sys.argv) > 1 else "c64_re")
...
def _package_for(rel: str) -> str:
    ...
    # c64_re/a/b.py -> package c64_re.a ; c64_re/a/__init__.py -> c64_re.a
    parts = rel[:-3].split("/")
    if parts[-1] == "__init__":
        parts = parts[:-1]
    else:
