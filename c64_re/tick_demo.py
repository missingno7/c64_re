...
A frame-keyed input demo advanced by a fixed INSTRUCTION budget per frame has a **mode-dependent**
clock: a recovered hook runs far fewer emulated instructions than the ASM it replaces, so the same
demo advances the game by a different amount in pure-ASM / hybrid / native mode — and the VM-less
native core has no instruction count at all.  (Proven on dos_re's first completed port: a pure-ASM
recording drifted in the hybrid while every gameplay hook verified byte-exact — the divergence was
purely the clock, never the logic.)

A **tick demo** is keyed to the GAME TICK instead — one main-loop iteration of the original game.
Per tick it stores the input the game actually *consumed* plus a digest of the gameplay state after
the tick.  Replaying steps exactly one tick and injects those inputs, so the same recording runs
IDENTICALLY in every mode, and the VM-less native core consumes it directly.  The tick index itself
is the only clock — mode-independent by design; that property is this module's whole point:

    record (VM = the oracle, any mode)          verify (no VM at all)
    ────────────────────────────────────        ─────────────────────────────────
    per game tick:                              per game tick:
      inputs the game sampled                     inject recorded inputs + sidebands
      sideband values (see below)                 run ONE native tick
      digest(gameplay state)                      assert digest matches recording

If the native core reproduces the digest at EVERY tick, the VM-less game provably reproduces the
original byte-for-byte over the whole recording.  This is the proof the engine-flip rests on (the
porting method's "full demo corpus passes native-vs-VM tick-by-tick" exit condition).

Three hard-won rules are baked into this module's shape (each cost dos_re's source port a real
divergence):

* **Capture at the consumption point.**  Inputs are captured where the tick CONSUMES them, not
  where the frame starts — an IRQ delivering a keyboard-matrix / joystick change between the two
  changes the value *after* it was consumed and the recording then lies.  The adapter places the
  observer(s); a later observer simply overwrites the pending value (the "refine" pattern: capture
  a safe base early, overwrite with the exact consumed value at the true read site).
* **Sidebands: record-and-inject what the native core cannot reproduce.**  State derived from the
  instruction count, CIA timer phase, or raster position (an idle-fidget timer fed by the jiffy
  IRQ, a frame-skew counter) has no VM-less equivalent.  Record its per-tick value from the VM and
  inject it before each native tick, or the digest diverges on state the gameplay logic only
  *reads*.
* **The digest boundary is the ownership map.**  Digest exactly the state the gameplay tick OWNS:
  exclude render-only state, input plumbing, and async audio — the same mask the forward lockstep
  oracle proves byte-exact, so a digest match means "same gameplay" by the same definition.
  :func:`masked_digest` is the mask-applying helper; the offsets are the adapter's (they ARE the
  game knowledge).

Inputs on the C64 are joystick masks and pressed key names — during recording the drive delivers
them through ``machine.set_joy1`` / ``set_joy2`` / ``key_down`` / ``key_up``; the adapter's
observers encode whatever the game consumed into the fixed-length per-tick key record, and its
native ``inject`` decodes them back.  This module never interprets the record.

Everything game-specific — seam PCs, the input-cell list, the exclusion mask, the tick function and
its transition outcomes — is adapter-supplied.  This module owns the container + on-disk format,
the trace-hook recorder plumbing, and the inject → tick → compare loop.  Where dos_re used
checkpoints and overlay-segment gating around its seams, c64_re keeps the seam abstract: plain PCs
plus adapter callables — a flat 64K address space needs no segment gate, and repositioning uses
:func:`replay_to` (or a :mod:`c64_re.snapshot` capture) instead of checkpoint machinery.
"""
...
def masked_digest(region: "bytes | bytearray | memoryview", *,
    ...
    sprite slot").  Out-of-range offsets are ignored so one mask can serve differently-sized captures."""
...
class TickDemo:
    ...
    ``seed`` is the full VM RAM image at the first tick boundary (the native bootstrap state); per tick:
    the consumed input bytes (``keys``, fixed record length — joystick masks and/or key codes, adapter
    encoding), the post-tick gameplay digest (``digests``), and any named u16 ``sidebands``
    (record-and-inject channels for state the native core cannot reproduce — see the module docstring)."""
    ...
    _MAGIC = b"C64TICKD"     # v1: seed + keys + sha1 digests + named u16 sideband channels
    ...
    def suffix(self, start: int, seed: bytes) -> "TickDemo":
        """A sub-demo beginning at tick ``start``, with ``seed`` as its new bootstrap state.  The
        divergence-repro workflow: when :func:`verify_ticks` reports tick ``i``, reposition a fresh state
        with :func:`replay_to` (fast — no digest checks), capture its bytes as the new seed, and save
        ``demo.suffix(i, seed)``: subsequent runs then reproduce the divergence at tick 0 instead of
    ...
    def load(cls, path) -> "TickDemo":
        ...
        if raw[:8] != cls._MAGIC:
            raise ValueError(f"{path}: not a c64_re tick demo (bad magic {raw[:8]!r})")
        off = 8
...
def record_ticks(rt, *, seed_pc: int, commit_pc: int,
    ...
    The framework owns the plumbing: it installs a ``cpu.trace_fn`` recorder that watches the adapter's
    seam PCs and assembles the :class:`TickDemo`.  The adapter owns the game knowledge:

    * ``seed_pc`` — the main-loop top; the FIRST time it executes, the full RAM image is captured as
      the demo's seed (the native bootstrap state).
    * ``observe`` — ``{pc: callback(pending, rt)}`` capture sites.  Callbacks fill/overwrite the shared
    ...
    * ``commit_pc`` — the end-of-tick site.  ``commit(pending, rt)`` returns ``(keys, sidebands)`` to
      ...
      every commit call.
    * ``advance_one_frame()`` — advances the VM one frame (e.g. ``run_frames(rt, 1)``) and returns False
      when the drive is exhausted; the caller owns pacing/input (an input replay, a fast-forward driver,
      a live view feeding ``machine.set_joy2``/``key_down``).

    Cost, documented honestly: ``trace_fn`` fires before EVERY interpreted instruction, so recording
    runs at tracing-VM speed — this is the offline record mode, not the everyday workbench.  Caveat:
    ``trace_fn`` does not fire at addresses claimed by service or replacement hooks (hook dispatch
    precedes the trace in ``cpu.step``), so seam PCs must be plain instruction addresses — a seam that
    coincides with a hook is an adapter bug and records nothing.  An existing ``trace_fn`` is chained,
    ...
    def recorder(c, pc, opcode):
        if prev is not None:
            prev(c, pc, opcode)
        if pc == seed_pc and not seeded[0]:
            out.seed = bytes(mem.ram)
            seeded[0] = True
        elif pc == commit_pc and seeded[0]:
...
def verify_ticks(demo: TickDemo, state, *,
    ...
    unrecovered path should raise, which is reported as the divergence.
