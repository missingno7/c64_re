...
The collector consumes generic events: one call per interpreted instruction and one per hook
dispatch (verified / unverified / skipped — the verifier reports the measured original-instruction
count for verified calls). This module classifies addresses into adapter-named islands, accumulates
the counts, and renders the honest headline number every port reports:

    native % = hook-covered ASM-equivalent instructions
               ─────────────────────────────────────────────────────────────
               interpreted + bounded-original + hook-covered ASM-equivalent

Honesty rules, baked in (learned on the dos_re source ports):

* **Hooked work is measured in ASM-equivalent instructions**, not hook calls — one dispatch can
  replace a 3-instruction leaf or a 10,000-instruction decruncher, so counting calls flatters
  nothing and measures nothing. The verifier's oracle run measures it exactly for verified calls
  (:class:`c64_re.verification.VerifyOutcome` ``.oracle_instructions``); unverified calls use the
  per-hook average from a previous verified run (the JSON cache) and are reported as *estimated*;
  calls with no measurement at all are reported as **unmeasured**, outside the percentage — never
  guessed into it.
...
Wire-up — both feeds exist today without touching the core:

    from c64_re.coverage import CoverageCollector
    cov = CoverageCollector(classifier=my_classifier, cache_path=Path("artifacts/coverage_cache.json"))
    rt.cpu.trace_fn = lambda cpu, pc, op: cov.record_interpreted_instruction(pc)
    # hook side, from the verifier per call:
    #   verified:   cov.record_hook_verified(pc, name, outcome.oracle_instructions)
    #   unverified: cov.record_hook_unverified(pc, name)      (hybrid runs without the oracle)
    #   skipped:    cov.record_hook_skipped(pc, name)         (passthrough addresses)
    ... run a demo replay ...
    print(cov.format_summary())

Origin: faithful port of ``dos_re.coverage`` (itself the Overkill port's collector, promoted);
addressing translated from CS:IP pairs to flat 16-bit PCs, written ``$XXXX``.
"""
...
class HookStats:
    ...
    the verified calls (``VerifyOutcome.oracle_instructions`` per call); ``estimated_equiv``
    accumulates cache-average estimates for unverified calls."""
    addr: Addr
    name: str
    island: str
    calls: int = 0
...
class CoverageCollector:
    """Thread-safe collector for the coverage events (module docstring wire-up).
    ...
    def record_hook_verified(self, addr: Addr, name: str, oracle_instructions: int) -> None:
        """One verified hook call; ``oracle_instructions`` is
        :class:`c64_re.verification.VerifyOutcome` ``.oracle_instructions``."""
        if not self.enabled:
            return
        with self._lock:
            ...
            st.calls += 1
            st.verified_calls += 1
            st.asm_equiv += int(oracle_instructions)
            st.last_asm_equiv = int(oracle_instructions)
            self.total_hook_calls += 1
            isl = self._island(st.island)
            isl.hook_calls += 1
            isl.hook_equiv += int(oracle_instructions)
