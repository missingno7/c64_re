...
module provides the reusable runner behind ``tools/run_tests.py``:
...
It intentionally has no game imports.  Game-specific scripts may choose
which test files or functions to pass in, but the runner itself is generic.

Origin: ported unchanged in design from dos_re's ``dos_re/testing.py``.
"""
...
def _ensure_test_dir_on_path(root: Path, rel_path: Path) -> None:
    """Mirror pytest's rootdir insertion: a test file's own directory goes on
    sys.path so sibling imports (``from test_machine import ...``) resolve."""
    d = str((root / rel_path).resolve().parent)
    ...
    *,
    ...
    for pattern in patterns:
        for path in sorted(root.glob(pattern)):
            ...
            ensure_pytest_stub()
            _ensure_test_dir_on_path(root, path.relative_to(root))
...
def _call_test_function(root: Path, case: TestCase) -> None:
    ...
    ensure_pytest_stub()
    _ensure_test_dir_on_path(root, case.path)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="test function glob, e.g. 'test_c64_re_*'; may be repeated",
    )
